...
class TumblekaPlugin extends Omeka_Plugin_AbstractPlugin
{
	...
	**/
    protected $consumer_key = 'QQGtGIcGosIHYBU1GBex9itYPv4BpifEimjWRPRGyLh9R6wZtM';
    protected $consumer_secret = 'pLGk3Q8w7uqIpqCoCixq2cxJUBji8cMocKXibjYxEkfMbC5xDo';
    /**
	...
	{     
	    ...
	    {
			//$body = $body.'<a href="'.$item->getProperty('permalink').'"><img src="'.$item->getProperty('permalink').'"/></a>';       
	    	$body = $body . item_image_gallery(Array(),'square_thumbnail', false,$item);
	    	$body = $body.'<br>'.metadata($item,array('Dublin Core','Description'));
		}
	    
	    $tags = metadata($item,array('Dublin Core','Subject')).','.metadata($item,array('Dublin Core','Creator')).','.metadata($item,array('Dublin Core','Contributor'));
	...
	function registerNewTumblrAccount()
	{
		...
		cleanUpTumblrClient($client,$success);
	}
	
	function postToTumblr($blog,$newPost)
	{
		//echo "posting to tumblr!";
		//die("posting to tumblr");
		...
		if(($success = $client->Initialize()))
		{
			//THIS USES OPTION STORAGE RATHER THAN SESSION STORAGE...MAYBE TRY BOTH SOMEHOW?
			$client->access_token = get_option('tumbleka_token');
			$client->access_token_secret = get_option('tumbleka_secret');
	...
	function cleanUpTumblrClient($client,$success)
	{
